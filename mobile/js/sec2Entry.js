export const sec2Entry = () => {
  let hasRun = true;
  const headerTXT = document.querySelectorAll('.header-menu a');
  const headerBuger = document.querySelectorAll('.burger div');
  ...
  window.addEventListener('scroll', function() {
    ...
    if (scrollPos >= section2 - 50) {
      headerTXT.forEach(txt => {txt.style.color = '#9e9e9e';});
      headerBuger.forEach(burger => {burger.style.backgroundColor = '#9e9e9e';});
      ...
      headerTXT.forEach(txt => {txt.style.color = '#fff';});
      headerBuger.forEach(burger => {burger.style.backgroundColor = '#fff'})
    }
  });
  
}

function sec2particle(){
